...
SELECT 
    ...
    (  (
     ...
     WHERE AMOUNT_ADYEN <> AMOUNT_NETSUITE ---get only the records with difference in amounts in two sources
 ORDER BY 1
...
FROM 
  ...
  (
    SELECT 
    trans.ORDER_REF AS ORDER_REF, 
    trans.TRANDATE AS TRANSACTION_DATE, 
  ...
  (
      ...
      (
        ...
        FROM 
		  ...
		  (
            ...
            SUM(amount_foreign) AS REVENUE1 
            FROM 
				 (
				   ((dea.netsuite.TRANSACTION_LINES tranlines 
					INNER JOIN dea.netsuite.TRANSACTIONS trans on (tranlines.TRANSACTION_ID = trans.TRANSACTION_ID)) 
            ...
            trans.ORDER_REF, 
            trans.TRANDATE
          ) B ON (A.REFERENCE = B.REFERENCE1) AND (A.TRANSACTION_DATE = B.TRANSACTION_DATE1) 
     WHERE 
     GROSS <> REVENUE1---get only the records with difference in amounts in two sources
   ...
   (
    ...
    trans.TRANDATE
	  ...
	  E ON (D.REFERENCE = E.REFERENCE_NETSUITE)--transaction dates of missing data found do not match with dates in settlement file
  ) 
  J ON (H.ORDER_REF = J.REFERENCE_NETSUITE) AND (H.TRANSACTION_DATE = J.TRANSACTION_DATE_NETSUITE)
  ORDER BY 3